...
func main() {
	...
	verifyCode, err := NewGoogleAuth().VerifyCode(secret, code)
	if verifyCode {
		fmt.Println("verifyCode PASS")
	} else {
		fmt.Println("verifyCode ERROR", err)
	}
}
...
func initAuth(user string) (secret, code string) {
	// 秘钥
	secret = "ZE2FGXS5L3YE2QJI5AWMYNIT3F5QJBTH"
